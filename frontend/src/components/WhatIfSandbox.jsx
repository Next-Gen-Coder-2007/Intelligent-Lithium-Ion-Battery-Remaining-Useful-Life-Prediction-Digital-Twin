...
export default function WhatIfSandbox() {
  ...
  const [whatIfResult, setWhatIfResult] = useState({
    ...
    recommendations: [
      "Thermal Stress High: Active liquid cooling required to avoid accelerated SEI breakdown and catastrophic knee.",
      "Fast-Charging Overpotential: Step-down charging protocol (multi-stage CC-CV) recommended to reduce polarization heating.",
      "Depth of Discharge Warning: Restricting cycling depth between 15% - 85% can extend cycle lifespan by up to 2.3x."
    ],
    baseline_curve: [],
    stress_curve: []
  });
  ...
  const runSimulation = async () => {
    ...
    try {
      ...
      // Merge curves for unified chart
      const merged = [];
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 glass-panel rounded-2xl p-5 border border-slate-800">
        ...
        <div className="flex items-center gap-1.5 p-1 bg-slate-900 rounded-xl border border-slate-800 flex-wrap">
          <button
            ...
            className="px-2.5 py-1 rounded-lg text-xs font-mono text-slate-400 hover:text-white"
          >
            Nominal EV (25°C, 1C)
          </button>
          <button
            onClick={() => { setTestTemp(45); setTestCRate(2.5); setDodPct(100); }}
            className="px-2.5 py-1 rounded-lg text-xs font-mono text-amber-300 bg-amber-500/15 border border-amber-500/30 font-bold"
          >
            Extreme Fast-Charge (45°C, 2.5C)
          </button>
          <button
            onClick={() => { setTestTemp(0); setTestCRate(2.0); setDodPct(90); }}
            className="px-2.5 py-1 rounded-lg text-xs font-mono text-cyan-300 hover:text-white"
          >
            Sub-Zero Plating (0°C, 2C)
          </button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        {/* Life Reduction */}
        <div className="glass-panel rounded-2xl p-5 border border-rose-500/30 bg-gradient-to-br from-rose-950/20 via-slate-900 to-slate-900">
          <div className="flex items-center justify-between text-xs font-mono text-rose-400">
            <span className="flex items-center gap-1.5 font-bold">
              <TrendingDown className="w-4 h-4" /> LIFETIME LOSS
            </span>
            <span className="text-[10px] bg-rose-500/20 px-2 py-0.5 rounded text-rose-300">
              DEGRADATION PENALTY
            </span>
          </div>
          <div className="mt-2">
            ...
            <p className="text-xs text-slate-400 font-mono mt-1">
              Lost: <strong className="text-rose-400">{whatIfResult.lifetime_impact.cycle_loss} usable cycles</strong> before EOL
            </p>
          </div>
        </div>

        {/* Aging Acceleration Multiplier */}
        <div className="glass-panel rounded-2xl p-5 border border-amber-500/30 bg-gradient-to-br from-amber-950/20 via-slate-900 to-slate-900">
          <div className="flex items-center justify-between text-xs font-mono text-amber-400">
            <span className="flex items-center gap-1.5 font-bold">
              <Flame className="w-4 h-4" /> AGING ACCELERATION
            </span>
          ...
          <div className="mt-2">
            ...
            <p className="text-xs text-slate-400 font-mono mt-1">
              Relative to nominal 25°C 1.0C baseline cycling
            </p>
          </div>
        </div>
        ...
        <div className="glass-panel rounded-2xl p-5 border border-slate-800">
          <div className="flex items-center justify-between text-xs font-mono text-slate-400">
            <span className="flex items-center gap-1.5">
              <ShieldAlert className="w-4 h-4 text-cyan-400" /> EXPECTED EOL CYCLE
            </span>
            <span className="text-[10px] text-slate-500">1.40 Ah Threshold</span>
          </div>
          <div className="mt-2">
            <h3 className="text-3xl font-extrabold font-mono text-cyan-300">
        ...
        <div className="lg:col-span-4 glass-panel rounded-2xl p-5 border border-slate-800 space-y-4">
          ...
          <div>
            <div className="flex justify-between text-xs font-mono mb-1">
              <span className="text-slate-400">Ambient Chamber Temp (°C):</span>
          ...
          <div className="mt-4 pt-4 border-t border-slate-800/80">
            ...
            <div className="space-y-2">
              {whatIfResult.recommendations.map((rec, idx) => (
                <div key={idx} className="flex items-start gap-2 p-2 rounded-lg bg-slate-900/80 border border-slate-800 text-[11px] font-mono text-slate-300">
          ...
          <div className="flex items-center justify-between mb-3">
            ...
            <div className="flex items-center gap-3 text-[11px] font-mono">
              <span className="flex items-center gap-1 text-cyan-400">
                <span className="w-2 h-2 rounded-full bg-cyan-400 inline-block"></span> Baseline (25°C, 1C)
              </span>
              <span className="flex items-center gap-1 text-rose-400">
                <span className="w-2 h-2 rounded-full bg-rose-400 inline-block"></span> Stress ({testTemp}°C, {testCRate}C)
              </span>
            </div>
          </div>
          ...
          <div className="h-[280px] w-full">
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={combinedChartData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                ...
                <Line type="monotone" dataKey="baseline_capacity" stroke="#00F0FF" strokeWidth={2.5} dot={false} name="Baseline (25°C, 1.0C)" />
                <Line type="monotone" dataKey="stress_capacity" stroke="#EF4444" strokeWidth={2.5} dot={false} name={`Stress (${testTemp}°C, ${testCRate}C)`} />
              </LineChart>
            </ResponsiveContainer>
          </div>
        </div>
