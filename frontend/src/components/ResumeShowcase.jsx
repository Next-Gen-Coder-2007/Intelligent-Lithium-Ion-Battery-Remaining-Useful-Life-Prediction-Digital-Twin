...
export default function ResumeShowcase() {
  ...
  const resumeBullets = [
    ...
    },
    {
      ...
      text: "• Architected a production-ready Django REST Framework backend and responsive React (Vite + Tailwind CSS) dashboard, featuring live holographic cell visualization, 95% confidence intervals, and batch CSV processing, containerized via Docker for cloud deployment."
    }
  ];
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-slate-400 font-mono">
              Comprehensive technical architecture breakdown, mathematical formulation, and ready-to-use resume entries
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="glass-panel rounded-2xl p-5 border border-slate-800 space-y-4">
        ...
        <div className="space-y-3">
          {resumeBullets.map((bullet, idx) => (
            <div key={idx} className="bg-slate-900/90 rounded-xl p-4 border border-slate-800 space-y-2">
              <div className="flex items-center justify-between">
                <span className="text-xs font-mono font-bold text-cyan-300">{bullet.title}</span>
                <button
                  onClick={() => handleCopy(bullet.text, idx)}
                  className="flex items-center gap-1 px-2.5 py-1 rounded-lg bg-slate-800 hover:bg-slate-700 text-[11px] font-mono text-slate-300 border border-slate-700 transition-all"
                >
                  {copiedIndex === idx ? (
                    <>
              ...
              <pre className="text-xs text-slate-300 font-mono whitespace-pre-wrap leading-relaxed bg-slate-950 p-3 rounded-lg border border-slate-800/60">
                {bullet.text}
              </pre>
            </div>
          ))}
